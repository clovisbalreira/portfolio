...
// Nova função de tocar o Buzzer (apenas configura a execução)
void tocarBuzzer(int vezes, int tempoLigado, int tempoDesligado) {
  buzzerVezesTotal = vezes * 2; // Multiplica por 2 (liga e desliga)
  buzzerContagem = 0;
...
void gerenciarBuzzer() {
  if (buzzerContagem < buzzerVezesTotal) {
    ...
    if (atual - buzzerMillis >= intervalo) {
      ...
      // Se terminou todas as repetições, garante que o buzzer desligue
      if (buzzerContagem >= buzzerVezesTotal) {
...
void loop()
{
  ...
  // 2. ATUALIZAR RELÓGIO
  // O relógio só avança se comecarJogo for true, mas a função agora 
  // será chamada sempre para garantir o envio dos dados (enviarSerial)
  atualizarRelogio();

  // 3. LEITURA DO BOTÃO DE START/PRÓXIMO (Este sim pode ter o delay de 200ms)
  if (millis() - tempoComecarJogo > 200){
    tempoComecarJogo = millis();
    
    if (digitalRead(BUTTONS_INICIO) == LOW && millis() - tempoStart > 500) {
        tempoStart = millis();
        ...
        if (!comecarJogo) {
            if (intervalo) {
                ...
                Serial.println("{\"evento\":\"start\",\"tempoAtual\":2}");
            } else {
                // Se o jogo está parado, avisa o site para carregar as equipas
                Serial.println("{\"evento\":\"btn_fisico_proximo\"}");
            }
        }
    }
  }
...
void atualizarRelogio() {
  ...
  if (comecarJogo && !intervalo) {
    ...
    // Se o jogo está parado, mantemos o tempoRelogio sincronizado com o agora
    // para que, ao reiniciar, ele não saia contando segundos atrasados
    tempoRelogio = agora; 
  }
  enviarSerial();
}
...
void interpretarComando(String json) {
  ...
  // Estes comandos devem rodar mesmo que comecarJogo seja true
  ...
  if (json.indexOf("\"comando\":\"reset_gols\"") != -1) {
    ...
    segundos = 0;
    comecarJogo = false;
    intervalo = false; // IMPORTANTE: Resetar o intervalo também
  ...
  // 2. EQUIPES (Sempre reseta o estado interno ao receber novos nomes)
  if (json.indexOf("\"equipes\"") != -1) {
    comecarJogo = false; // Para o cronómetro se estiver a correr
    intervalo = false;
    minutos = 0;
    segundos = 0;
    tempoAtual = 1;
    ...
    totalEquipes = index;
    Serial.println("{\"debug\":\"equipes_prontas_aguardando_start\"}");
    return;
  }

  // 3. BLOQUEIO DE SEGURANÇA (Para comandos de START)
  // Se o jogo já está rodando, não processa o "start" novamente
  if (comecarJogo && (json.indexOf("\"start\"") != -1 || json.indexOf("\"comando\":\"start\"") != -1)) {
    return; 
  }

  // 4. COMANDO START
  if (json.indexOf("\"start\"") != -1 || json.indexOf("\"comando\":\"start\"") != -1) {
  ...
  if (json.indexOf("\"configuracao\"") != -1) {
    ...
    if (posTempo != -1) {
      ...
      String tempoStr = json.substring(inicio, fim); // Ex: "05:00"
      
      int separador = tempoStr.indexOf(":");
      if(separador != -1) {
        tempoMinutos = tempoStr.substring(0, separador).toInt();
        tempoSegundos = tempoStr.substring(separador + 1).toInt();
      ...
      Serial.print("{\"debug\":\"tempo_configurado\", \"min\":");
...
void zerarMinutos() {
  ...
  // Ex: Se configuraste 05:00, ele para quando minutos == 5 e segundos == 0
  if (minutos >= tempoMinutos && segundos >= tempoSegundos) {
    ...
    if (tempoAtual == 1) {
      ...
      comecarJogo = false;
      intervalo = false;
      tempoAtual = 1; // Reseta para o próximo jogo ser 1º tempo
...
void botaoAcionado(int i) {
  ...
  if (digitalRead(BUTTONS_JOGADORES[i]) == LOW && (millis() - tempoButton > 250)) {
    ...
    if (i < totalEquipes) { 
      ...
      Serial.print("{\"debug\":\"Botao pressionado\", \"jogador\":");
      Serial.print(i);
      Serial.println("}");
    ...
    // Trava para não contar múltiplos golos se segurar o botão
...
void confirmarGol(int i) {
  ...
  while(millis() - inicioVAR < 5000) {
    atualizarRelogio(); // CRÍTICO: Mantém o tempo rodando no site e no Arduino
    ...
    if(digitalRead(BUTTONS_JOGADORES[inimigo]) == LOW) {
      // GOL ANULADO!
      EQUIPE[i].gol--;
      Serial.print("{\"evento\":\"gol_anulado\",\"time\":");
      Serial.print(i);
      Serial.println("}");
      
      // Espera o adversário soltar o botão antes de continuar
      while(digitalRead(BUTTONS_JOGADORES[inimigo]) == LOW) {
        atualizarRelogio();
      }
      
      // Pequena pausa de 2 segundos de "luto" pelo gol anulado
...
int totalJogadores(){
  return sizeof(BUTTONS_JOGADORES) / sizeof(BUTTONS_JOGADORES[0]);
}
