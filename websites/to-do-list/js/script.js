...
function mostrarErro(frase){
    let erro = document.getElementById('erro')
    erro.innerHTML = frase
    erro.style.display = 'block'
    erro.style.background = 'red'
    erro.style.padding = '.4rem 1rem'
    erro.style.borderRadius = '2rem'
}
...
function newTask(){
    ...
    input.style.border = ''
    if(!input.value){
        mostrarErro('Digite algo para inserir na sua lista')
    }else if(validateIfExistsNewTask()){
        input.style.border = '1px solid red'
        mostrarErro('Já existe uma task com essa descrição')
    }else{
        let values = JSON.parse(localStorage.getItem(localStorageKey) || '[]')
        values.push({
            name: input.value
